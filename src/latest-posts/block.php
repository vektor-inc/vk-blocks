...
function vk_blocks_render_latest_posts( $attributes ) {
	...
	global $wp_query;
	$wp_query = vk_blocks_get_loop_query( $attributes );
	global $post;
	...
	if ( have_posts() ) :
		...
		while ( have_posts() ) {
			the_post();
			$elm .= '<li>';
			$elm .= vk_blocks_get_loop_post_view( $post );
			$elm .= '</li>';
		} // while ( have_posts() ) {
		$elm .= '</ul>';
		$elm .= '</div>';
...
function vk_blocks_get_loop_query( $attributes ) {
...
function vk_blocks_get_loop_post_view( $post ) {
	$elm          = '';
