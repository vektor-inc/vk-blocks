...
/**
 * Lightning テーマ依存の Highlighter（マーカー）テスト
 *
 * このテストはハイライト色を「Lightning」テーマのカラーパレットから取得する。
 * そのため Lightning テーマがテスト実行環境にインストール・有効化されている必要がある。
 *
 * テーマのインストールについて:
 * - 未インストールの場合は beforeAll で wordpress.org のテーマディレクトリから自動的に
 ...
 */
test.describe('Lightning Highlighter', () => {
	// 各テスト前に Lightning テーマをインストール（未インストール時のみ）して有効化する。
	// page は test スコープのフィクスチャのため beforeAll では使えず、beforeEach で行う。
	test.beforeEach(async ({ requestUtils, page }) => {
		await installAndActivateTheme(page, requestUtils, 'lightning');
	});
	...
	test('Lightning Highlighter', async ({ admin, editor, page }) => {
		// 新しく投稿を作る
		...
		// 以下の 'More' / 'Highlighter' / 'Black' は英語 UI ロケール前提のアクセシブルネーム。
		// 他言語ロケールでは表示文言が変わるため、本テストは英語 UI 環境での実行を前提とする。
		...
		// 色オプション「Black」を選択する（Lightning パレット由来 → #000000）
		...
		expect(await editor.getEditedPostContent()).toBe(
			`<!-- wp:paragraph -->
<p><span data-color="#000000" style="--vk-highlighter-color: rgba(0, 0, 0, 0.7);" class="vk_highlighter">1</span></p>
<!-- /wp:paragraph -->`
		);
	});
});
